<script lang="ts">
  ...
  async function confirm(contentId: string) {
    ...
    try {
      data = await submitEntry({ contentId, ign, role });
...
<main class="page-shell">
  ...
  {:else if data}
    {#each data.expansions as expansion}
      <section class="expansion-section" aria-labelledby={`${expansion.id}-heading`}>
        <div class="section-heading">
          <h2 id={`${expansion.id}-heading`}>{expansion.name}</h2>
          <span>{expansion.shortName}</span>
        </div>

        {#each expansion.categories as category}
